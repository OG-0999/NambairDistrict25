...
export function FloorPlans() {
  ...
  const renderPlanCards = (items: typeof unitPlans) => (
    <div className="flex flex-wrap items-stretch gap-6">
      {items.map((plan, idx) => (
        ...
        >
          <div className="flex w-full min-h-[360px] flex-col overflow-hidden border border-white/10 bg-white/[0.03] backdrop-blur-lg shadow-[0_20px_60px_rgba(0,0,0,0.35)] transition-all duration-500 group-hover:-translate-y-1 group-hover:border-primary/40 group-hover:shadow-[0_30px_80px_rgba(201,169,110,0.25)]">
            ...
            <div className="flex flex-1 flex-col items-center justify-between px-6 py-6 text-center">
              <div>
                <h3 className="text-xl font-serif text-white tracking-[0.08em]">{plan.title}</h3>
                <p className="mt-2 text-xs uppercase tracking-[0.25em] text-white/50">{plan.subtitle}</p>
              </div>
              <button className="mt-6 w-full max-w-[220px] border border-primary/40 bg-primary/10 px-6 py-2.5 text-xs uppercase tracking-[0.3em] text-primary transition-all duration-500 hover:bg-primary hover:text-black hover:shadow-[0_0_25px_rgba(201,169,110,0.45)]">
                Enquire Now
              </button>
            </div>
          </div>
  ...
  return (
    <section id="floorplans" className="py-24 bg-black">
      <div className="container mx-auto px-6">
        ...
        >
          <h2 className="text-3xl md:text-4xl font-serif text-white mb-4">UNIT PLANS</h2>
        ...
        >
          <h3 className="text-2xl md:text-3xl font-serif text-white mb-4">FLOOR PLANS</h3>
          <div className="h-[1px] w-24 bg-primary mx-auto"></div>
        </motion.div>
...
export function Location() {
  return (
    <section id="location" className="py-24 bg-[#050505] border-y border-white/5">
      <div className="container mx-auto px-6">
        <motion.div 
          initial="hidden"
          whileInView="visible"
        ...
        >
          <h2 className="text-3xl md:text-4xl font-serif text-white mb-6 leading-tight">AN ADDRESS IN BENGALURU'S FASTEST GROWING NEIGHBOURHOOD</h2>
        ...
        <div className="flex flex-col gap-8 max-w-4xl mx-auto">
          <motion.div 
            ...
            className="space-y-6"
          >
            <div className="bg-black border border-white/10 p-5 flex items-start gap-4 hover:border-primary/50 transition-colors">
              <div className="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center shrink-0 text-primary mt-1">🚇</div>
              <div>
                <h4 className="text-white font-serif text-lg mb-2">Connectivity</h4>
                <p className="text-white/60 text-sm">600m from Muthanallur Metro Station (upcoming Red Line)<br/>Near Wipro SEZ, Cisco, ITPL, Electronic City</p>
              </div>
            </div>
            ...
            <div className="bg-black border border-white/10 p-5 flex items-start gap-4 hover:border-primary/50 transition-colors">
              <div className="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center shrink-0 text-primary mt-1">🏫</div>
              <div>
                <h4 className="text-white font-serif text-lg mb-2">Education</h4>
                <p className="text-white/60 text-sm">TISB, Oakridge International, Bethany High, Greenwood High</p>
              </div>
            </div>
            ...
            <div className="bg-black border border-white/10 p-5 flex items-start gap-4 hover:border-primary/50 transition-colors">
              ...
              <div>
                <h4 className="text-white font-serif text-lg mb-2">Retail & Leisure</h4>
                <p className="text-white/60 text-sm">Forum Value Mall, Central Mall, SOHO Zone</p>
              </div>
            </div>
            
            <p className="text-white/40 text-xs mt-8">Chandapura Dommasandra Road, Dommasandra, Bengaluru - 562 125</p>
          </motion.div>
        </div>
      </div>
    </section>
    ...
    <section className="py-24 bg-black overflow-hidden relative">
      ...
      <div className="container mx-auto px-6">
        ...
        >
          ...
          <h2 className="text-3xl md:text-4xl font-serif text-white">THE GENERATIONAL ADDRESS</h2>
        </motion.div>
        ...
        <div className="flex overflow-x-auto snap-x snap-mandatory gap-6 pb-8 hide-scrollbar">
          {reviews.map((rev, idx) => (
            <motion.div
              ...
              className="min-w-[320px] md:min-w-[400px] bg-white/[0.03] border border-white/5 p-6 snap-center hover:bg-white/[0.05] transition-colors"
            >
              <div className="flex text-primary mb-6">
                {[...Array(rev.stars)].map((_, i) => <span key={i}>★</span>)}
              </div>
              <p className="text-white/80 font-light text-base mb-8 leading-relaxed">"{rev.text}"</p>
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="text-white font-medium">{rev.name}</h4>
                  <span className="text-white/40 text-sm">{rev.role}</span>
                </div>
              </div>
            </motion.div>
          ))}
